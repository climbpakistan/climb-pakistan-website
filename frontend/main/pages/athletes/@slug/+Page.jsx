...
import { personSchema } from '../../../src/utils/jsonLd';
    ...
    return (
      <section className="page-header">
        <div className="container">
          ...
          <a href="/athletes" className="btn btn-primary" style={{ marginTop: 'var(--sp-6)' }}>Back to Athletes</a>
        </div>
      </section>
    );
  }
            ...
            <div className="athlete-hero-info hero-entrance hero-entrance-delay-1">
              ...
              {(athlete.instagram || athlete.worldClimbingUrl) && (
                <div className="athlete-hero-actions">
                  {athlete.instagram && (
                    <a href={`https://instagram.com/${athlete.instagram}`} className="btn-instagram" target="_blank" rel="noopener noreferrer">
                  ...
                  {athlete.worldClimbingUrl && (
                    <a href={athlete.worldClimbingUrl} className="btn-world-climbing" target="_blank" rel="noopener noreferrer">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="athlete-detail-main">
              {showFilter && (
                <div className="filter-bar" role="tablist" aria-label="Filter medals by discipline">
                  <button className={`filter-chip${filterDiscipline === 'All' ? ' is-active' : ''}`} role="tab" aria-selected={filterDiscipline === 'All'} onClick={() => setFilterDiscipline('All')}>All</button>
                  {medalDisciplines.map((d) => (
                    <button key={d} className={`filter-chip${filterDiscipline === d ? ' is-active' : ''}`} role="tab" aria-selected={filterDiscipline === d} onClick={() => setFilterDiscipline(d)}>{d}</button>
                  ))}
                </div>
              )}
              ...
              {filterDiscipline === 'All' && (athlete.medals?.length || 0) > 0 && (
                <div className="medal-summary">
                  {medalCounts.gold > 0 && (<div className="medal-count"><span className="medal-count-icon" aria-hidden="true"><MedalIcon type="gold" size={22} /></span><span className="medal-count-num">{medalCounts.gold}</span></div>)}
                  {medalCounts.silver > 0 && (<div className="medal-count"><span className="medal-count-icon" aria-hidden="true"><MedalIcon type="silver" size={22} /></span><span className="medal-count-num">{medalCounts.silver}</span></div>)}
                  {medalCounts.bronze > 0 && (<div className="medal-count"><span className="medal-count-icon" aria-hidden="true"><MedalIcon type="bronze" size={22} /></span><span className="medal-count-num">{medalCounts.bronze}</span></div>)}
                </div>
              )}

              <div className="medals-display">
                {filteredMedals.length === 0 && (<p style={{ color: 'var(--cp-text-muted)' }}>No medals recorded for this discipline.</p>)}
                {filteredMedals.map((m, i) => (
                  <div className="medal-row" key={i} style={{ opacity: 0, transform: 'translateY(16px)', animation: `fadeInUp 0.45s ease ${0.15 + i * 0.08}s forwards` }}>
                    <span className="medal-icon" aria-hidden="true"><MedalIcon type={MEDAL_MAP[m.medal] || 'gold'} size={26} /></span>
                    <div><p className="medal-competition">{m.competition}</p><p className="medal-meta">{m.discipline} · {m.medal}</p></div>
                  </div>
                ))}
              </div>
            ...
            <aside className="athlete-detail-side">
              <div className="info-card">
                ...
                <dl className="info-list">
                  ...
                  {athlete.instagram && (<div><dt>Instagram</dt><dd><a href={`https://instagram.com/${athlete.instagram}`} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--cp-accent)' }}>@{athlete.instagram}</a></dd></div>)}
                </dl>
              </div>
            </aside>
          </div>
