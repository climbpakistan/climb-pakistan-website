  ...
  const rows = [
    ...
    <div className="form-row" key="subject">
      ...
      <select id="subject" name="subject" defaultValue="Story Tip">
        <option>Story Tip</option><option>Athlete Submission</option><option>Correction</option><option>General Inquiry</option>
      </select>
    </div>,
      ...
      <form className="contact-form" onSubmit={handleSubmit}>
        {Children.map(rows, (child, i) =>
          isValidElement(child) ? cloneElement(child, {
            style: { ...(child.props.style || {}), opacity: isVisible ? 1 : 0, transform: isVisible ? 'translateY(0)' : 'translateY(16px)', transition: `opacity 0.45s ease ${0.08 + i * 0.06}s, transform 0.45s ease ${0.08 + i * 0.06}s` },
          }) : child
        )}
        <input type="checkbox" name="botcheck" className="hidden" style={{ display: 'none' }} tabIndex="-1" autoComplete="off" />
        <button type="submit" className="btn btn-primary" disabled={status.state === 'sending'}
          style={{ opacity: isVisible ? 1 : 0, transform: isVisible ? 'translateY(0)' : 'translateY(16px)', transition: `opacity 0.45s ease 0.32s, transform 0.45s ease 0.32s` }}
        >
  ...
  return (
    <>
      <Seo
        ...
        keywords="contact Climb Pakistan, climbing Pakistan contact, sport climbing Pakistan inquiries, Pakistan climbing team contact, story tip Pakistan climbing, athlete submission Pakistan climbing, Pakistan climbing community"
        path="/contact"
      />
